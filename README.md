...
This project was generated by [TraceAI](https://www.trae.ai), so you know~🤣.
...
- **Parallel chunked transfer**: Optimized transfer for large files (>1MB)
...
2. **Chunked Transfer**: Supports parallel chunked transfer for large files (>1MB) to improve transfer speed
...
You can test the synchronization functionality using the provided PowerShell script:

```powershell
./test_network_sync.ps1
```

This script starts a server and client process locally to test the synchronization functionality.
